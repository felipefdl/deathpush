...
pub async fn clone_repository(
  ...
  GitCli::clone_repo(&url, &target, &event_sink).await?;
  ...
  if let Some(sink) = &event_sink {
    if let Err(err) = watcher::start_watcher(&label, sink.clone(), &repo_root, &watcher_state) {
...
pub async fn init_repository(
  ...
  let target = PathBuf::from(&path);
  let event_sink = get_event_sink();
  GitCli::init_repository(&target, &event_sink).await?;
