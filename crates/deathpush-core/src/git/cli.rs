...
fn emit_git_command(event_sink: &Option<Arc<dyn EventSink>>, args_str: &str, duration_ms: u64) {
  if let Some(sink) = event_sink {
    let now = chrono::Local::now();
    sink.emit_git_command(
      &format!("git {args_str}"),
      duration_ms,
...
impl GitCli {
  ...
  pub fn with_event_sink(repo_root: &Path, event_sink: Arc<dyn EventSink>) -> Self {
    Self {
      git_path: PathBuf::from("git"),
      repo_root: repo_root.to_path_buf(),
  ...
  pub async fn run(&self, args: &[&str]) -> Result<String> {
    ...
    if output.status.success() {
      ...
      Err(Error::GitCli { message: stderr })
    }
  }
  ...
  pub async fn stash_apply(&self, index: u32) -> Result<()> {
    self.run(&["stash", "apply", &format!("stash@{{{}}}", index)]).await?;
    Ok(())
  }

  pub async fn stash_pop(&self, index: u32) -> Result<()> {
    self.run(&["stash", "pop", &format!("stash@{{{}}}", index)]).await?;
    Ok(())
  }

  pub async fn stash_drop(&self, index: u32) -> Result<()> {
  ...
  pub async fn clone_repo(url: &str, path: &Path, event_sink: &Option<Arc<dyn EventSink>>) -> Result<()> {
    let start = Instant::now();
  ...
  pub async fn stash_show(&self, index: u32) -> Result<String> {
    self
      .run(&["stash", "show", "-p", &format!("stash@{{{}}}", index)])
      .await
  }

  pub async fn init_repository(path: &Path, event_sink: &Option<Arc<dyn EventSink>>) -> Result<()> {
    let start = Instant::now();
    let output = async_command("git")
