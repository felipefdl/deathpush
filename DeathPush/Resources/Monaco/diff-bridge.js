// diff-bridge.js -- Bridge between Swift WKWebView and Monaco Diff Editor
// Communicates with Swift via window.webkit.messageHandlers.diffBridge
...
(function () {
  ...
  // Disable noisy language features for diff viewing
  ...
  function sendToSwift(type, payload) {
    if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.diffBridge) {
      window.webkit.messageHandlers.diffBridge.postMessage({ type, ...payload });
    }
  }
  ...
  window.setDiffContent = function (content) {
    if (!diffEditor) return;
    require(["vs/editor/editor.main"], function (monaco) {
      const lang = content.originalLanguage || getLanguageForPath(content.path);
      const original = monaco.editor.createModel(content.original || "", lang);
      const modified = monaco.editor.createModel(content.modified || "", lang);
